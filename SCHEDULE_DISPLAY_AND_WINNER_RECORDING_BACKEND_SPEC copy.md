...
| B3 | `record-winner` on a fresh Type 64 group, first call | `auction_number = 2` (month 1 skipped), `bid_amount = 326250` — NOT `auction_number = 1` / `0` |

---

## Implementation notes / corrections (added 2026-07-15)

Reviewed against a proposed backend implementation plan. The plan is faithful to
this spec on structure (B1, B2, B4, the dual-endpoint duplicate guard, the
shared winner-tracking helper). Two corrections and two easy-to-miss guards must
be folded in — each produces wrong data otherwise. Verified against current
backend code.

### C1 — `getSchemeWinningAmount` does not exist to "extract" — create it, and share it with the member display

The winning-amount function is **not** currently in `userService.js`.
`getBidDetailsService` (~L654/660) and `getChitDetailsService` (~L985) still
return raw `parseFloat(auction.bid_amount)` — i.e.
`docs/FIXED_CHIT_MEMBER_DISPLAY_SPEC.md` was never implemented. So there is
nothing to extract; the function must be **created fresh** (it is fully specced
in that member-display doc).

Because B3 (`record-winner`) and the member-facing display use the **exact same
formula**, implement `getSchemeWinningAmount` once (e.g. `src/utils/schemeHelpers.js`)
and call it from **all three** sites: `record-winner`, `getBidDetailsService`,
and `getChitDetailsService`. Otherwise the admin stores a correct winning amount
while members keep seeing the raw `bid_amount`. **Recommend implementing
`FIXED_CHIT_MEMBER_DISPLAY_SPEC.md` as part of this work.**

### C2 — Company-month skip applies to Type 64, not only Type 63

The auto-assigned `auction_number` must skip company months for **both** fixed
types that have them:

```
companyMonths = company_chit   // Type 63
              = 1              // Type 64  (month 1 is the company chit; prices[0].chit_amount is a 0 placeholder)
              = 0              // Types 62, 65, and Open Auction
auction_number = MAX(lastRecorded, companyMonths) + 1
```

If Type 64 is not skipped, a fresh Type 64 group's first winner gets
`auction_number = 1`, and `getSchemeWinningAmount` reads the month-1 placeholder
(`0`) instead of month 2's real payout (₹3,26,250). This is the C1 formula's
input — the two corrections interact.

### C3 — Two guards the plan omitted (both already in step 4/7 above)

- Reject when `auction_number > no_of_installments` (scheme already finished).
- Update `ChitsGroup.auction_date` from `next_auction_date` when provided —
  parity with `store-or-update`, so the UI's "next auction date" keeps advancing.

### C4 — Transaction safety + guard ordering (beyond this spec, but do it here)

- `record-winner` inserts an `Auction` **and** mutates the winner's future
  installments. Wrap both in a single DB transaction — otherwise a failure after
  the insert leaves an auction with no adjustment applied, and the new duplicate
  guard then blocks any retry. (`storeOrUpdateAuctionService` is not
  transactional today either — fix it while here.)
- On the `isNew` path, the duplicate-winner guard MUST run **before** the
  winner-tracking increment — that ordering is what prevents the live Type-63
  double-bump.
- Note (not blocking): `record-winner` writes money and the admin routes have no
  role check (see the security review). Keep authorization on the backlog.
