...
Covers four new features added to the frontend in this sprint:

1. **Super Admin — Terms & Privacy** (platform-level, no company scope)
2. **Super Admin — Support Page** (platform-level Contact Info + FAQs)
3. **Public — Terms & Privacy viewer** (Android/web WebView)
4. **Public — Support** (Android WebView)
5. **Public — Delete Account** (Android app WebView flow)
...
| 2 | `/terms` | Public (Android WebView / web) | Default API token |
| 3 | `/privacy` | Public (Android WebView / web) | Default API token |
| 4 | `/support` | Public (Android WebView) | Default API token |
| 5 | `/delete-account` | Public (Android WebView) | Default API token |

Each page cross-links to the other (`/terms` has a "Privacy Policy →" link and `/privacy` has a "← Terms & Conditions" link).
...
### 1.3 `POST public/terms-privacy`

**Auth:** `authenticateDefaultToken` (same as login — no user session needed)

This is the **public read endpoint** called by the `/terms` and `/privacy` frontend pages
(opened by the Android app WebView). It reads from the same `platform_terms_privacy`
table that the super admin writes to via 1.1 and 1.2 above.
...
**Response (not published yet — return empty content, NOT 404):**
```json
{
  "status": 1,
  "message": "Success",
...
> The frontend handles an empty `content` gracefully — it shows "This document has
> not been published yet." A 404 would show a generic error instead, so always
> return `status: 1` with an empty content string when the row doesn't exist.
...
| `POST public/terms-privacy` | `authenticateDefaultToken` | `platform_terms_privacy` (read-only) |
